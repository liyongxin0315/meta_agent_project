...
def temp_test_dir(temp_project_root: Path) -> Generator[Path, None, None]:
    ...
    Yields:
        Path: 函数级临时测试目录路径
    """
    test_dir = temp_project_root / f"test_{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S%f')}"
...
class TestDataBuilder:
    ...
    def create_defect(
        ...
        """
        return {
            "id": f"defect_{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}",
            "component": component,
    ...
    def create_modification(
        ...
        """
        return {
            "id": f"mod_{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}",
            "action": action,
            "component": component,
            "description": description,
            "content": {"test": "data"},
